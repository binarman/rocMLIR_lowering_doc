...
\begin{figure}[!h]
  ...
  \begin{tikzpicture}
    ...
    \foreach \tile [count=\t from 0] in {1,...,\k}{
      ...
      \coordinate (tileCTL) at ($(tileBLDSTL)+(0, -2*\gapH-\cellUnitH)$);
      \coordinate (tileALDSTL) at ($(tileBLDSTL)+(.5*\cellUnitW-.5*\cellUnitH,\gapH+\cellUnitW)$);
      %% blockwise_gemm op boundary
      \draw [ultra thin, fill=gray!50!white] ($(tileCTL)+(-.1, -\cellUnitW-.1)$) rectangle ++(.2+\cellUnitW, 3.65);
      %% tile C in register
      \draw [fill=\tileCCol] (tileCTL) rectangle ++(\cellUnitW, -\cellUnitW);
      %% tile A and tile B in LDS
      \draw [fill=\tileACol] (tileALDSTL) rectangle ++(\cellUnitH, -\cellUnitW) node [pos=.5] {\t};
    ...
    \draw [thick, dashed] (RFTL) rectangle ++(\LDSW+3*\cellUnitW, -\RFH);
    
    %% accumulate register C tile
    \coordinate (AccTileCTL) at ($(tileCTL)+(\cellUnitW+2*\cellUnitW, 0)$);
    \draw [fill=purple] (AccTileCTL) rectangle ++(\cellUnitW, -\cellUnitW);
    \draw [->, >=stealth] ($(tileCTL)+(\cellUnitW+.2, -.5*\cellUnitW)$) -- ++(2*\cellUnitW-.4, 0)
    node [above, pos=.5, scale=.7] {accumulate};

    %% copy register tile C to matrix C
    \draw [fill=\tileCCol] ($(matrixCTL)+(\nt*\cellUnitW, -\mt*\cellUnitW)$) rectangle ++(\cellUnitW, -\cellUnitW);
    \draw [->, >=stealth] ($(AccTileCTL)+(\cellUnitW, 0)$) to[out=45, in=-90]
    ($(matrixCTL)+(\nt*\cellUnitW+.5*\cellUnitW, -\mt*\cellUnitW-\cellUnitW)$);

    \node [align=center, inner sep=0] (blockwise) at ($(tileALDSTL)+(3, 0)$) {blockwise\_gemm op\\arguments};
    \draw [->, >=stealth] ($(blockwise.south west)+(.3, .3)$) -- ++(-1.1, -.8);
     
  \end{tikzpicture}
  \caption{Gemm computation task for one workgroup (CUDA block).}
  \label{fig:2d_fwd_conv_stride_2}
\end{figure}
