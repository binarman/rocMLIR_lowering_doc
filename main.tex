...
\begin{figure}[!h]
  ...
  \begin{tikzpicture}
    ...
    %\draw ($(bufferBTL)+(0, -\kPerBlock*\unit)$) -- ++(\mPerMfmaGroup*\unit,0);
    %% draw invalid part of buffer B
    %\draw ($(bufferBTL)+(\nPerMfmaGroup*\unit,0)$) rectangle ++(\nPerMfmaGroup*\unit, -\kPerBlock*\unit*\nRepeats);
    %\draw [dashed] ($(bufferBTL)+(\nPerMfmaGroup*\unit, -\kPerBlock*\unit)$) -- ++(\nPerMfmaGroup*\unit,0);
    ...
    %\draw [fill=cyan] (mfmaTL) circle (2pt);
    ...
    %\draw ($(mfmaTL)+(.6, -1.15)$) -- ++(10, 0);
    %\draw ($(mfmaTL)+(.6, -5.75)$) -- ++(10, 0);
    ...
    %% connect the matrixA and matrixB for the mfma instruction
    %\draw [fill=cyan] (mfmaTL2) circle (2pt);
    %\draw [->, >=stealth] ($(bufferATL)+(0, -1.5*\kBase*\unit)$) to[out=-135, in=90]
    %($(mfmaTL)+(.6, -1.15)$);
    %\draw [->, >=stealth] ($(bufferBTL)+(0, -6.5*\kBase*\unit)$) to[out=-135, in=0]
    %($(mfmaTL)+(5.5, -.9)$);
    ...
    %\coordinate (bufferCTL) at ($(matrixCDTL)+(1.6, -.8)$);
    %\foreach \tid in {0,...,52} {
    %  \draw [fill opacity=.2, fill=\tileCCol, draw opacity=.2] ($(bufferCTL)+(\tid*\unit, 0)$) rectangle ++(\unit, -5.43);
    %}
    \foreach \tid in {0,...,52} {
